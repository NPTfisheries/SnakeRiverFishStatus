...
# set default coordinate reference system for this analysis
default_crs = st_crs(4326) # WGS84
...
sr_int_sites_sf = ptagis_sf %>%
  ...
  filter(!str_detect(site_name, "Lower Granite|LOWER GRANITE|Little Goose|Lower Monumental")) %>% # remove select dam sites, dealt with later
  ...
  filter(!site_code %in% c("0HR", # Henry's Instream Array, Lemhi
                           ...
                           # Dworshak natural-origin fish are returned back to river and so we don't really know their destination unless they are detected 
                           # elsewhere which is not necessarily the case for other "Hatchery Return" facilities (e.g., CRT, RRT, STL, STR) where we can be 
                           # confident that they stay in that tributary
...
sr_site_pops = crb_sites_sf %>%
  ...
  mutate(
    ...
    sthd_popname = if_else(site_code == "SFG", "South Fork Salmon River/Secesh River", sthd_popname)
  )
...
save(configuration,
     ...
     flowlines,
     parent_child,
     file = paste0("data/configuration_files/site_config_LGR_", format(Sys.Date(), "%Y%m%d"), ".rda"))
